...
def vector(weights, visibility, grid_indices, Data, Data_scale, Phase, cg_calc_mode):
    if cg_calc_mode == 'Monodisperse':
        return vector_monodisperse(weights, visibility, grid_indices, Data, Data_scale)
    elif cg_calc_mode == 'Polydisperse':
        return vector_polydisperse(weights, visibility, grid_indices, Data, Data_scale, Phase)
    else:
        raise ValueError(f"Unknown cg_calc_mode: {cg_calc_mode}. Expected 'Monodisperse' or 'Polydisperse'.")
...
def tensor(weights, visibility, grid_indices, Data1, Data2, Data_scale, Phase, cg_calc_mode):
    if cg_calc_mode == 'Monodisperse':
        return tensor_monodisperse(weights, visibility, grid_indices, Data1, Data2, Data_scale)
    elif cg_calc_mode == 'Polydisperse':
        return tensor_polydisperse(weights, visibility, grid_indices, Data1, Data2, Data_scale, Phase)
    else:
        raise ValueError(f"Unknown cg_calc_mode: {cg_calc_mode}. Expected 'Monodisperse' or 'Polydisperse'.")
